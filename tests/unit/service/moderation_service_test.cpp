...
TEST_F(ModerationServiceTest, HandleModerationResult_FlaggedSavesToRepo) {
    ...
    service_->HandleModerationResult(response, test_constants::TEST_ID_BASIC, "original text",
                                     moderation::OBJECT_TYPE_COMMENT_TEXT);
    ASSERT_EQ(1U, repo_->GetStoredRecords().size());
    EXPECT_EQ(test_constants::TEST_ID_BASIC, repo_->GetStoredRecords()[0].object_id);
...
TEST_F(ModerationServiceTest, ProcessModerationRequest_ValidRequest_SendsToKafka) {
    EXPECT_TRUE(
        service_->ProcessModerationRequest(test_constants::TEST_ID_SECONDARY, "hello world"));

    ASSERT_EQ(1U, kafka_->GetSentRequests().size());

    const auto& sent = kafka_->GetSentRequests()[0];
    EXPECT_EQ(test_constants::TEST_ID_SECONDARY, sent.id());
...
TEST_F(ModerationServiceTest, HandleModerationResult_NotFlagged_DoesNotSave) {
    ...
    service_->HandleModerationResult(response, test_constants::TEST_ID_BASIC, "clean text",
                                     moderation::OBJECT_TYPE_COMMENT_TEXT);
...
TEST_F(ModerationServiceTest, ProcessModerationRequest_SendFails_ReturnsFalse) {
    ...
    EXPECT_FALSE(service_->ProcessModerationRequest(test_constants::TEST_ID_BASIC, "test text"));
...
TEST_F(ModerationServiceTest, HandleModerationResult_SavesFlaggedContent) {
    ...
    service_->HandleModerationResult(response, test_constants::TEST_ID_SECONDARY, "bad content",
                                     moderation::OBJECT_TYPE_COMMENT_TEXT);

    ASSERT_EQ(1U, repo_->GetStoredRecords().size());

    const auto& record = repo_->GetStoredRecords()[0];
    EXPECT_EQ(test_constants::TEST_ID_SECONDARY, record.object_id);
...
TEST_F(ModerationServiceTest, HandleModerationResult_RepositoryFailure_LogsError) {
    ...
    service_->HandleModerationResult(response, test_constants::TEST_ID_BASIC, "content",
                                     moderation::OBJECT_TYPE_COMMENT_TEXT);
