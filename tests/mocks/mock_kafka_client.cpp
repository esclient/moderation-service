#include "tests/mocks/mock_kafka_client.hpp"
#include "mock_kafka_client.hpp"

MockKafkaClient::MockKafkaClient(const KafkaConfig& config) : KafkaClient(config) {}

void MockKafkaClient::Initialize(std::function<void(const moderation::ModerateObjectResponse&,
                            int64_t, const std::string&, moderation::ObjectType)> callback) {
...
void MockKafkaClient::SimulateResponse(const moderation::ModerateObjectResponse& response,
                                    int64_t request_id, const std::string& original_text, moderation::ObjectType type) {
    if(callback_)
    {
