...
bool ModerationRepository::SaveModerationResult(const ModerationRecord& result) {
    try {
        ...
        transaction.exec_params("INSERT INTO moderation_records (object_id, object_type, text, "
                                "is_flagged, reason, moderated_at) "
                                "VALUES ($1, $2, $3, $4, $5, to_timestamp($6))",
                                result.object_id, static_cast<int>(result.object_type), result.text,
                                result.is_flagged, result.reason, seconds);
        transaction.commit();
        return true;
    } catch (const std::exception& e) {
